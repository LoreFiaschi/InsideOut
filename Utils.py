...
def add_lat_lon_to_nodes_df(nodes_df: pd.DataFrame, utm_zone=32, northern=True):
    ...
    print("CRS for Berlin v5.5 only!")
...
def export_to_kepler(file: str, link_df: pd.DataFrame, city_name, utm_zone=32, northern=True, interventions_df=None):
    ...
    if not interventions_df is None:
...
def get_trip_origins(activities_df: pd.DataFrame):
    ...
    # filter activities that are freight or car/ride interactions
    mask = ~(origins.type.str.contains("interaction") | origins.type.str.contains("freight"))
    origins = origins[mask].copy()

    # for each plan id, drop the last item, this is not a trip origin
    def drop_last(group):
        return group.iloc[:-1]
...
def add_nodeId_to_zones(zones_df: pd.DataFrame, nodes_df: pd.DataFrame):
    ...
    # Do not use PT nodes! (not connected to street network!?)
...
def get_from_and_to_links_per_node(links_df: pd.DataFrame, exclude_pt=True):
    ...
    # from nodes
    from_df = links_df.groupby("from_node").apply(lambda grp: list(grp.link_id.values))  # out_links
    from_df = from_df.to_frame().reset_index()
    from_df["from_idx"] = links_df.groupby("from_node").apply(lambda grp: list(grp.index.values)).values  # out_links
    from_df.rename(columns={"from_node": "node_id", 0: "from_links"}, inplace=True)

    # to nodes
    to_df = links_df.groupby("to_node").apply(lambda grp: list(grp.link_id.values))  # in_links
    to_df = to_df.to_frame().reset_index()
    to_df["to_idx"] = links_df.groupby("to_node").apply(lambda grp: list(grp.index.values)).values  # out_links
    to_df.rename(columns={"to_node": "node_id", 0: "to_links"}, inplace=True)

    # is all from now also to node
    ...
    print(f"len(diff1): {len(diff1)} and len(diff2): {len(diff2)}")

    if len(diff1) or len(diff2):
        print("WARINING: From and to nodes different. Cannot simply merge.")
...
def get_largest_scc(link_df: pd.DataFrame, nodes_df: pd.DataFrame):
    ...
    # Convert the largest strongly connected component to a subgraph
    ...
    print(f"SCC Edges: {len(link_df):,d}. SCC Nodes: {len(nodes_df):,d}")
...
def get_in_and_out_degree_per_node_df(nodes_df, link_df):
    ...
    # for nodes that we not mentioned --- fill with 0
    res.fillna(0, inplace=True)

    return res


# TODO make faster, but since run so irregularly... not an issue.
def simplify_network(link_df, node_df):

    # get in and out degrees
    in_out_df = get_in_and_out_degree_per_node_df(node_df, link_df)

    # get in == out degree == 1 nodes
    ...
    # create new edges and flag to be deleted ones.
    new_link_df = link_df.copy()
    new_link_df["remove"] = False
    for node in tqdm(in_out_1_nodes):
        ...
        # fetch in out links for details --- take last (possible that newly added needs to be delted too)
        in_link = new_link_df[in_link_mask].iloc[-1]
        out_link = new_link_df[out_link_mask].iloc[-1]

        # # will be removed at a later stage? --- yes if from link or two link is in the list
        # remove = in_link.from_node in in_out_1_nodes
        # remove = remove or (out_link.to_node in in_out_1_nodes)

        # combine both --- set a new one
        # new_links.append(
    ...
    # remove links that were to be removed.
